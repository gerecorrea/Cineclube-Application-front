						...
						>
							<template slot="table-row" slot-scope="props">
								...
								<span v-else-if="props.column.field == 'action'">
									<button class="mdi mdi-star-off md-color-yellow-700" uk-tooltip="Retirar nota" @click="removeRate(props.row.uuid)"></button>
									<button v-if="props.row.favorite"
										class="mdi mdi-heart md-color-red-700"
										uk-tooltip="Desfavoritar"
										@click="props.row.favorite = !props.row.favorite, changeFavorite(props.row.uuid)"
									></button>
									<button v-else-if="!props.row.favorite"
										class="mdi mdi-heart-outline md-color-red-700"
										uk-tooltip="Favoritar"
										@click="props.row.favorite = !props.row.favorite, changeFavorite(props.row.uuid)"
									></button>
									<button v-if="props.row.watchlist"
										class="mdi mdi-bookmark-check md-color-yellow-800"
...
export default {
	...
	methods: {
		...
		listRatings () {
			...
			UserMovieRelation.findRatingsUser(this.loggedUserObject.uuid)
				.then(response => {
		...
		removeRate (uuid){
			UserMovieRelation.changeRating(uuid, 0)
				.then(response => {
					let message = "Retirada nota do filme!";
			...
			UserMovieRelation.changeFavorite(uuid)
				.then(response => {
					...
					if (response.data.favorite) 
						message = "Filme retirado dos favoritos!";
